...
router.post("/servicerequest", verify, async (req, res) => {
  ...
  try {
    ...
    if (error) return res.status(400).send(error.details[0].message);
      ...
      //NEW SERVICE REQUEST  IS ADDED IN THE COLLECTION 'ticket'
      ...
      const mailData = {
        subject: "New Service Request Added by Admin",
...
router.put("/servicerequest/:id", async (req, res) => {
  try {
    ...
    const email = req.body.senderEmail;
    const mailData = {
      subject: "Service Request Updated by Admin",
...
router.post("/lead", verify, async (req, res) => {
  ...
  try {
    ...
    if (error) return res.status(400).send(error.details[0].message);
      ...
      const email = req.body.senderEmail;
      const mailData = {
        subject: "New Lead Added by Admin",
...
router.post("/contact", verify, async (req, res) => {
  console.log("came in admin add contact api");
